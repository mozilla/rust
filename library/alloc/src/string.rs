...
impl String {
    ...
    pub unsafe fn from_raw_parts(buf: *mut u8, length: usize, capacity: usize) -> Self {
        unsafe { Self { vec: Vec::from_raw_parts(buf, length, capacity) } }
    }
}
...
impl<A: AllocRef> String<A> {
    ...
    pub fn with_capacity_in(capacity: usize, alloc: A) -> Self {
        Self { vec: Vec::with_capacity_in(capacity, alloc) }
    }
    ...
    /// Creates a new `String` from a length, capacity, pointer, and allocatorand.
    ///
    /// # Safety
    ///
    /// This is highly unsafe, due to the number of invariants that aren't
    ...
    pub unsafe fn from_raw_parts_in(
        ...
        unsafe { Self { vec: Vec::from_raw_parts_in(buf, length, capacity, alloc) } }
    }
...
#[stable(feature = "box_str2", since = "1.45.0")]
impl<A1: AllocRef, A2: AllocRef> Extend<Box<str, A2>> for String<A1> {
...
macro_rules! impl_eq {
    ($lhs:ty, $rhs: ty, $($Arg: ident),*) => {
        #[stable(feature = "rust1", since = "1.0.0")]
        #[allow(unused_lifetimes)]
        impl<'a, 'b, $($Arg: AllocRef),*> PartialEq<$rhs> for $lhs {
            #[inline]
        ...
        #[stable(feature = "rust1", since = "1.0.0")]
        #[allow(unused_lifetimes)]
        impl<'a, 'b, $($Arg: AllocRef),*> PartialEq<$lhs> for $rhs {
            #[inline]
...
impl_eq! { String<A>, str, A }
impl_eq! { String<A>, &'a str, A }
impl_eq! { Cow<'a, str>, str, }
impl_eq! { Cow<'a, str>, &'b str, }
impl_eq! { Cow<'a, str>, String<A>, A }
...
impl<A: AllocRef + Clone> From<&String<A>> for String<A> {
    #[inline]
    fn from(s: &Self) -> Self {
        s.clone()
    }
...
impl<A: AllocRef> From<Box<str, A>> for String<A> {
    ...
    fn from(s: Box<str, A>) -> Self {
        let slice: Box<[u8], A> = s.into();
        Self { vec: <[u8]>::into_vec(slice) }
    }
}
...
/// [`drain`]: String::drain
#[stable(feature = "drain", since = "1.6.0")]
pub struct Drain<'a, #[unstable(feature = "allocator_api", issue = "32838")] A: AllocRef = Global> {
