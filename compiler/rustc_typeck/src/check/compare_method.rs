...
crate fn compare_impl_method<'tcx>(
    ...
    if let Err(ErrorReported) = compare_const_generic_types(tcx, impl_m, trait_m, trait_item_span) {
        return;
    }
}
...
fn compare_const_generic_types<'tcx>(
    tcx: TyCtxt<'tcx>,
    impl_m: &ty::AssocItem,
    trait_m: &ty::AssocItem,
    trait_item_span: Option<Span>,
    ...
    for (const_param_impl, const_param_trait) in iter::zip(const_params_impl, const_params_trait) {
        ...
        if impl_ty != trait_ty {
            let (impl_span, impl_ident) = match tcx
                .hir()
                .find(tcx.hir().local_def_id_to_hir_id(const_param_impl.expect_local()))
            {
            ...
            let trait_span = match const_param_trait.as_local() {
                Some(local_def_id) => {
                    match tcx.hir().find(tcx.hir().local_def_id_to_hir_id(local_def_id)) {
                        Some(hir::Node::GenericParam(hir::GenericParam { span, .. })) => Some(span),
                        Some(other) => bug!("expected GenericParam, found {:?}", other),
                        None => None,
                    }
                }
                None => None,
