...
fn prepare_vtable_segments<'tcx, T>(
    ...
    // For a multiple inheritance relationsihp like this,
    //   D --> C --> A
    //           \-> B
    // The resulting vtable will consists of these segments:
    //  DSA, A, B, B-vptr, C, D

    // For a diamond inheritance relationsihp like this,
    //   D --> B --> A
    //     \-> C -/
    // The resulting vtable will consists of these segments:
    //  DSA, A, B, C, C-vptr, D
    ...
    let mut stack = vec![(trait_ref, emit_vptr_on_new_entry, None)];
                ...
                'exiting_out_skip_visited_traits: loop {
                    if let Some(siblings) = siblings_opt {
                        if let Some(next_inner_most_trait_ref) = siblings.next() {
                            ...
                            stack.pop();
                            break 'exiting_out_skip_visited_traits;
                        }
                    } else {
                        stack.pop();
                        break 'exiting_out_skip_visited_traits;
    ...
    let vtable_segment_callback = |segment| -> ControlFlow<()> {
        match segment {
            ...
            VtblSegment::TraitOwnEntries { trait_ref, emit_vptr } => {
                ...
                if emit_vptr {
                    let trait_ref =
                        ty::ExistentialTraitRef::erase_self_ty(tcx, trait_ref.skip_binder());
                    entries.push(VtblEntry::TraitVPtr(ty::Binder::dummy(trait_ref)));
                }
            }
        }
