...
/// stored/returned.
#[inline]
fn validate_hir_id_for_typeck_results(hir_owner: HirOwner, hir_id: hir::HirId) {
...
#[cold]
#[inline(never)]
fn invalid_hir_id_for_typeck_results(hir_owner: HirOwner, hir_id: hir::HirId) {
    ty::tls::with(|tcx| {
        bug!(
...
impl<'tcx> TypeckResults<'tcx> {
    pub fn new(hir_owner: HirOwner) -> TypeckResults<'tcx> {
        TypeckResults {
            hir_owner,
...
impl<'a, 'tcx> HashStable<StableHashingContext<'a>> for TypeckResults<'tcx> {
    fn hash_stable(&self, hcx: &mut StableHashingContext<'a>, hasher: &mut StableHasher) {
        ...
        hcx.with_node_id_hashing_mode(NodeIdHashingMode::HashDefPath, |hcx| {
            hcx.local_def_path_hash(hir_owner.def_id);
...
pub struct GlobalCtxt<'tcx> {
    ...
    trait_map: FxHashMap<HirOwner, FxHashMap<ItemLocalId, StableVec<TraitCandidate>>>,
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn in_scope_traits(self, id: HirId) -> Option<&'tcx StableVec<TraitCandidate>> {
        self.in_scope_traits_map(id.owner.def_id).and_then(|map| map.get(&id.local_id))
    }

    pub fn named_region(self, id: HirId) -> Option<resolve_lifetime::Region> {
        debug!(?id, "named_region");
        self.named_region_map(id.owner.def_id).and_then(|map| map.get(&id.local_id).cloned())
    }

    pub fn is_late_bound(self, id: HirId) -> bool {
        self.is_late_bound_map(id.owner.def_id)
            .map_or(false, |(owner, set)| owner == id.owner.def_id && set.contains(&id.local_id))
    }

    pub fn object_lifetime_defaults(self, id: HirId) -> Option<Vec<ObjectLifetimeDefault>> {
        self.object_lifetime_defaults_map(id.owner.def_id)
    }

    pub fn late_bound_vars(self, id: HirId) -> &'tcx List<ty::BoundVariableKind> {
        self.mk_bound_variable_kinds(
            self.late_bound_vars_map(id.owner.def_id)
    ...
    pub fn lifetime_scope(self, id: HirId) -> Option<LifetimeScopeForPath> {
        self.lifetime_scope_map(id.owner.def_id).and_then(|mut map| map.remove(&id.local_id))
    }
}
...
pub fn provide(providers: &mut ty::query::Providers) {
    providers.in_scope_traits_map = |tcx, id| tcx.gcx.trait_map.get(&HirOwner { def_id: id });
