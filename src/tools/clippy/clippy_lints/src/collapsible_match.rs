...
use clippy_utils::{higher, is_lang_ctor, path_to_local, SpanlessEq};
use if_chain::if_chain;
use rustc_hir::LangItem::OptionNone;
use rustc_hir::{Expr, ExprKind, Guard, HirId, Pat, PatKind, StmtKind, UnOp};
...
fn check_if_let<'tcx>(cx: &LateContext<'tcx>, outer_expr: &'tcx Expr<'tcx>, outer_pat: &'tcx Pat<'tcx>) {
    ...
    if_chain! {
        ...
        if let Some(binding_id) = path_to_local(strip_ref_operators(inner_let_expr, cx.typeck_results()));
        if let Some(binding_span) = find_pat_binding(outer_pat, binding_id);
        let mut used_visitor = LocalUsedVisitor::new(cx, binding_id);
